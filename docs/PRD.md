...
| **4. Parsing via LLM** | `parser`                 | Converte o texto bruto (`textos_brutos/`) diretamente em JSON canônico usando o Gemini. O resultado final é salvo em `textos_estruturados/` e `status_parsing` é atualizado. | -                           |
...
Quando houver necessidade de registrar feedback manual, a tabela `llm_parser_sugestao` permanece disponível para receber observações sobre ajustes desejados, mas o comparativo determinístico-versus-LLM foi suspenso nesta fase.
