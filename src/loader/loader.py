...
def insert_postgres(conn, data):
    ...
    try:
        date_obj = datetime.strptime(ato['data_publicacao'], '%d/%m/%Y')
        formatted_date = date_obj.strftime('%Y-%m-%d')
    except (ValueError, TypeError):
        # Se a data já estiver no formato correto ou for inválida, usa como está ou None
        formatted_date = ato.get('data_publicacao')
    ...
    cursor.execute(
        ...
        (ato['urn_lexml'], ato['tipo_ato'], ato['titulo'], ato.get('ementa'), ato['entidade_federativa'], ato['orgao_publicador'], formatted_date, ato['fonte_oficial'], ato['situacao_vigencia'])
    )
    ato_id = cursor.fetchone()[0]
    print(f"Ato Normativo inserido com ID: {ato_id}")

    # 2. Inserir Dispositivos e Versões (recursivamente)
    def insert_dispositivos_recursivo(dispositivos, parent_id, p_formatted_date):
        for disp in dispositivos:
            ...
            for versao in disp['versoes']:
                cursor.execute(
                    ...
                    (disp_id, p_formatted_date, versao.get('vigencia_fim'), versao.get('texto_original_parser'), versao['texto_normalizado'], versao['hash_texto_normalizado'], versao['status_vigencia'], versao['texto_normalizado'])
                )

            if disp.get('filhos'):
                insert_dispositivos_recursivo(disp['filhos'], disp_id, p_formatted_date)

    # A chave no JSON é 'dispositivos' (plural) e é uma lista
    if 'dispositivos' in data:
        insert_dispositivos_recursivo(data['dispositivos'], None, formatted_date)
...
def insert_neo4j(driver, data):
    ...
    with driver.session() as session:
        # 1. Usar MERGE para o Ato Normativo
        session.run("MERGE (a:AtoNormativo {urn_lexml: $urn}) ON CREATE SET a.titulo = $titulo, a.tipo_ato = $tipo_ato", 
                    urn=urn_ato, titulo=ato['titulo'], tipo_ato=ato['tipo_ato'])
        print(f"Nó AtoNormativo garantido para URN: {urn_ato}")
        ...
        def insert_dispositivos_recursivo(dispositivos, parent_id_unico):
            for disp in dispositivos:
                ...
                # Usar MERGE para o Dispositivo
                session.run("""
                    MERGE (d:Dispositivo {id_unico: $id_unico})
                    ON CREATE SET d.caminho_estrutural = $caminho, d.rotulo = $rotulo, d.tipo = $tipo
                    """, id_unico=id_unico_disp, caminho=disp['caminho_estrutural'], rotulo=disp['rotulo'], tipo=disp['tipo_dispositivo'])
                
                # Usar MERGE para a relação com o pai
                if parent_id_unico:
                    session.run("""
                ...
                # Usar MERGE para a Versão e sua relação
                for versao in disp['versoes']:
                    session.run("""
                        MATCH (d:Dispositivo {id_unico: $id_unico})
                        MERGE (v:VersaoTextual {hash: $hash})
                        ON CREATE SET v.texto = $texto, v.status = $status
                        MERGE (d)-[:POSSUI_VERSAO]->(v)
...
def main(input_path):
    ...
    try:
        ...
        print(f"Erro na carga do PostgreSQL: {e}", file=sys.stderr)
        sys.exit(1)
    finally:
